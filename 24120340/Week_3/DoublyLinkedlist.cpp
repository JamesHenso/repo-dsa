...
void removeBefore(List *&L, int val)
{
    if (L->p_head == nullptr || L->p_head->key == val || L->p_head->p_next == nullptr)
        return;
...
bool addPos(List *&L, int data, int pos)
{
    ...
    while (temp != nullptr && visited < pos - 1)
    {
        temp = temp->p_next;
        visited++;
    }
...
void removePos(List *&L, int data, int pos)
{
    ...
    while (temp != nullptr && visited < pos)
    {
        temp = temp->p_next;
        visited++;
    }
...
void removeDuplicate(List *&L)
{
    ...
    while (current != nullptr)
    {
        if (seen.count(current->key))
        {
            NODE *toDelete = current;
            current = current->p_next;

            if (toDelete->p_prev != nullptr)
                toDelete->p_prev->p_next = toDelete->p_next;
            else
                L->p_head = toDelete->p_next;
